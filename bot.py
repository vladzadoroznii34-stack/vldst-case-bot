...
async def start(m:Message):
    ...
    with db() as con:
        ...
        con.commit()
    kb=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='🎁 Открыть VLDST CASE',web_app=WebAppInfo(url=WEBAPP_URL))]])
...
async def admin_cmd(m:Message):
    ...
    kb=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='⚙️ Открыть ADMIN',web_app=WebAppInfo(url=WEBAPP_URL+'/admin'))]])
